...
class Circle
{
	...
	public:
		...
		{
			if (r > 0 && (x0 - r > 0) && (y0 - r > 0) && x0 > 0 && y0 > 0)
			{
				_r = r;
				_x0 = x0;
				_y0 = y0;
			...
			{
				throw std::invalid_argument(" radius is smaller 0 or circle isn`t at XOY");
			}
		}
		~Circle()
		{
...
void Circle::setRadius(double radius)
{
	...
	{
		throw std::invalid_argument("Radius is smaller than 0");
	}
}
...
int Circle::setCenterx(double x0)
{
	...
	{
		throw std::invalid_argument("x0 + r isn`t at XOY");
	}
	return (0);
}
...
int Circle::setCentery(double y0)
{
	...
	{
		throw std::invalid_argument("y0 + r isn`t at XOY");
	}
	return (0);
}
...
void area_check(Circle &C, double k)
{
	...
	if (x0 + sqrt(k) * r > 0 && y0 + sqrt(k) * r > 0)
	{
		...
		throw std::invalid_argument("if you set radius k*r , x0 + k*r or y0 + k*r isn`t at XOY");
